...
"""
    ...
    edgeRolloff = pexConfig.ConfigField(dtype=EdgeRolloffConfig,
                                        doc=EdgeRolloffConfig.__doc__)
...
def makeExposure(inputFile, weightFile, varianceFile, badPixelValue, variance, verbose):
    ...
    if np.isfinite(variance):
        ...
        if variance <= 0:
            if verbose:
                print "estimating variance from the image"
            mi = exposure.getMaskedImage()
